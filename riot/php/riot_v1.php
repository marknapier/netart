...
// print "########### addToCache elements=" . $elements . " safeurl=" .$safeURL;
...
function logURL($safeURL) {
	...
	// print "############## logfilecontents=" . $logFileContents . "\n";

	// if decode failed (urls is null), return an empty array
	if (!$urls) {
		$urls = array();
	}
...
function latestUrls() {
	...
	// strip off the var declaration so we have just the array of urls
	$logFileContents = preg_replace('/var riotUrlHistory = /', '', $logFileContents);
	// convert string with array of urls into actual array
	$urls = $logFileContents ? json_decode($logFileContents) : array();
	// if decode failed (urls is null), return an empty array
	if (!$urls) {
		$urls = array();
